...
uint16_t WS2812FX::mode_noize_fire(void) {
  static uint32_t prevTime;
  if (millis() - prevTime > 30) {
    prevTime = millis();
    for (uint16_t i = _seg->start; i <= _seg->stop; i++) {
      CRGB color = ColorFromPalette(firePalette, (inoise8(i * 15, counter)), 255, LINEARBLEND);
      setPixelColor(i, color.g, color.r, color.b);
    }
    counter += 20;
  }
  return _seg->speed / 30;
}
